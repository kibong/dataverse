...
@register_etl
def cleaning___content___bad_words_filter(
    spark,
    data: Union[RDD, DataFrame],
    subset: str = "text",
    *args,
    **kwargs
    ...
    """
    Filters the data by removing entries that contain bad or spam words.

    Args:
        spark (SparkSession): The Spark session object.
        data (Union[RDD, DataFrame]): The input data to be processed.
        subset (str, optional): The subset or column to consider for filtering. Defaults to 'text'.

    Returns:
        RDD: The filtered data as an RDD, excluding entries with bad or spam words.
    """
    if isinstance(data, DataFrame):
        data = data.rdd
        data = data.map(lambda row: row.asDict())
    ...
    filtered_data = data.filter(lambda row: not contains_bad_words(row[subset]))
